      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <section className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          <div className="mt-3 flex flex-col sm:flex-row gap-2">
            <input aria-label="Advisor question" value={advisorQuestion} onChange={(event) => setAdvisorQuestion(event.target.value)} placeholder="What are we underestimating?" className="flex-1 bg-black border border-gray-700 rounded p-3 text-sm focus:border-cyan-500 focus:outline-none"/>
        ...
        {showLedger && <section className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          <div className="mt-5 grid md:grid-cols-2 gap-5">
            <div><div className="text-xs uppercase font-mono text-blue-400 mb-2">Resources</div>{visibleState.resources.length ? visibleState.resources.map((resource) => <div key={resource.id} className="text-sm text-gray-400 mb-2"><span className="text-gray-200">{resource.label}</span>: {resource.amount} {resource.unit}<div className="text-[11px] text-gray-600">Owner: {resource.ownerName} · {resource.renewable ? 'renewable' : 'finite'}</div></div>) : <p className="text-sm text-gray-600">No visible resources.</p>}</div>
            <div><div className="text-xs uppercase font-mono text-purple-400 mb-2">Relationships</div>{visibleState.relationships.length ? visibleState.relationships.map((relationship) => <div key={relationship.id} className="text-sm text-gray-400 mb-2"><span className="text-gray-200">{relationship.fromName} → {relationship.toName}</span><div className="text-[11px] text-gray-600">Alignment {relationship.alignment} · Trust {relationship.trust} · Leverage {relationship.leverage} · {relationship.communication ? 'channel open' : 'no direct channel'}</div>{relationship.commitments.map((commitment) => <div key={commitment} className="text-[11px] text-amber-700">Commitment: {commitment}</div>)}</div>) : <p className="text-sm text-gray-600">No visible relationships.</p>}</div>
            <div><div className="text-xs uppercase font-mono text-amber-400 mb-2">Active arcs</div>{visibleState.arcs.length ? visibleState.arcs.map((arc) => <div key={arc.id} className="text-sm text-gray-400 mb-2"><span className="text-gray-200">{arc.title}</span>: {arc.progress}/{arc.threshold} · {arc.direction.toLowerCase()}<div className="text-[11px] text-gray-600">{arc.description}{arc.dueTurn ? ` · Due turn ${arc.dueTurn}` : ''}</div></div>) : <p className="text-sm text-gray-600">No visible active arcs.</p>}</div>
            <div><div className="text-xs uppercase font-mono text-cyan-400 mb-2">Known processes</div>{visibleState.processes.length ? visibleState.processes.map((process) => <div key={process.id} className="text-sm text-gray-400 mb-2"><span className="text-gray-200">{process.label}</span>: {process.progress}/{process.requiredProgress}<div className="text-[11px] text-gray-600">Due turn {process.dueTurn}</div></div>) : <p className="text-sm text-gray-600">No visible ongoing processes.</p>}</div>
          </div>
          <div className="mt-5"><div className="text-xs uppercase font-mono text-blue-400 mb-2">Known facts and provenance</div>{knownFacts.map((fact) => <div key={fact.id} className="text-sm text-gray-400 mb-2">• {fact.statement} <span className="text-gray-600">({fact.confidence})</span><div className="ml-3 text-[11px] font-mono text-gray-600">{fact.provenance.replaceAll('_', ' ').toLowerCase()}{fact.sourceRefs.length ? ` · ${fact.sourceRefs.join('; ')}` : ''}</div></div>)}</div>
          {visibleState.beliefs.length > 0 && <div className="mt-5"><div className="text-xs uppercase font-mono text-emerald-400 mb-2">Intelligence estimates</div><div className="grid md:grid-cols-2 gap-2">{visibleState.beliefs.map((belief) => <div key={`${belief.subjectId}.${belief.field}`} className="bg-gray-950 border border-gray-800 rounded p-3 text-xs"><div className="text-gray-300">{belief.subjectId.replaceAll('_', ' ')} · {belief.field.replaceAll('_', ' ')}</div><div className="text-gray-500 mt-1">{belief.range ? `${belief.range[0]}–${belief.range[1]}` : belief.estimate ?? belief.categorical ?? 'uncertain'} · {belief.confidence.toLowerCase()} confidence</div></div>)}</div></div>}
        </section>}
